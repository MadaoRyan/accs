 ...
 */
void
__deskey(DES_KS k, unsigned char *key, int decrypt)
/*       k: Key schedule array */
 ...
 */
void
__des3key(DES3_KS k, unsigned char *key, int decrypt)
/*       k: Key schedule array */
/*     key: 192 bits (will use only 168) */
/* decrypt: 0 = encrypt, 1 = decrypt */
{
	if(!decrypt){
		__deskey(&k[0],&key[0],0);
		__deskey(&k[16],&key[8],1);
		__deskey(&k[32],&key[16],0);
	} else {
		__deskey(&k[32],&key[0],1);
		__deskey(&k[16],&key[8],0);
		__deskey(&k[0],&key[16],1);
	}
}
