...
void DSPToHEX(strDsp, strHex, nCount)
char *strDsp, *strHex;
int nCount;
{
    int i;
...
void HEXToDSP(strHex, strDsp, nCount)
char *strDsp, *strHex;
int nCount;
{
    int i;
    ...
    for(i = 0; i < nCount; i++)
    {
        ...
        strDsp[i * 2] = (cTemp > 9) ? cTemp + 0x41 - 10 : cTemp + 0x30;
        cTemp = strHex[i] & 0xf;
...
void Do_DES(ustrSource,ustrDest,ustrKey,uchFlag)
UNCHAR* ustrSource;
UNCHAR* ustrDest;
UNCHAR* ustrKey;
UNCHAR* uchFlag;
{
	...
	if (ustrSource == NULL || ustrKey == NULL || ustrDest == NULL)
		return;
		
	expand0(ustrKey, tmp);
	setkeystar(tmp);
	if( uchFlag == (UNCHAR*)ENCRYPT || uchFlag == (UNCHAR*)'e' || uchFlag == (UNCHAR*)'E' )
		encrypt0(ustrSource, ustrDest);
	else
		discrypt0(ustrSource, ustrDest);
