...
int txn90002Initial(char *pData,T_HEAD t_Head);
int txn90002Process();
int txn90002nd();
...
int txn90002(char *pData,T_HEAD t_Head,char *pOutData,char *pErrNo)
{
	...
	logMsg(LOG_TRC,"90002平台资金查询业务，处理开始");

    nRet = txn90002Initial(pData,t_Head);
    if (nRet != 0)
    {
    ...
    memcpy(pOutData+sizeof(f_sPlatformId),t_Rep.sBalance  ,18                     );
	return 0;
}

int txn90002Initial(char *pData,T_HEAD t_Head)
{
	T_TXN90002 *t_txn = (T_TXN90002 *) pData;
...
int txn90002Process()
{
	...
	/*平台账户余额md5校验*/
	sprintf(sSqlCmd,"select amount from TG_PLATACCINFO where platformid = '%s' "
...
int txn90002End()
{
	logMsg(LOG_TRC,"90002平台资金查询业务，处理结束");
	return 0;
}
