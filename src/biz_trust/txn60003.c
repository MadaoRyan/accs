...
int txn60003Initial(char *pData,T_HEAD t_Head);
int txn60003Process();
int txn60003End();
...
int txn60003(char *pData,T_HEAD t_Head,char *pOutData,char *pErrNo)
{
	...
	logMsg(LOG_TRC,"60003提现业务，处理开始");

    nRet = txn60003Initial(pData,t_Head);
    if (nRet != 0)
    {
...
int txn60003Initial(char *pData,T_HEAD t_Head)
{
	T_TXN60003 *t_txn = (T_TXN60003 *) pData;
...
int txn60003Process()
{
	...
	{
		/*账号还未绑定*/
		logMsg(LOG_ERR,"外部账号还未进行绑定，不能提现[%s]",f_sAccId);
	...
	/*修改个人账户*/
	nRet = UpdateAcc(sAccountCode,sAccCode,"4",f_sAmount,f_sFee);
	if(nRet == 2)
	{
	...
	{
		logMsg(LOG_ERR,"账户余额不足，不能提现[%d]",nRet);
		strcat(f_sError,"E0000002");
		return -1;
	}
	else if(nRet < 0)
	{
		logMsg(LOG_ERR,"提现失败[%d]",nRet);
		return -1;
	}

	/*记录平台账户*/
	nRet = UpdatePlatAcc(f_sPlatformId,"2",f_sAmount,f_sFee);
	if(nRet == 2)
	{
		logMsg(LOG_ERR,"账户余额校验失败[%d]",nRet);
		strcat(f_sError,"E0000010");
